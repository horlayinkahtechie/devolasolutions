...
import { MdOutlineDesignServices, MdAllInclusive } from "react-icons/md";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {services.map((s) => (
              <div
                id={`service-${s.num}`}
                key={s.num}
                className={`relative rounded-3xl border ${s.card} ${s.text} p-8 md:p-10 flex flex-col gap-8 group hover:shadow-2xl hover:-translate-y-1 transition-all duration-500`}
              >
                {/* Header */}
                <div className="relative z-10 flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span
                      className={`inline-flex items-center px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${s.tag}`}
                    >
                      {s.title}
                    </span>
                    <span className={`text-xs font-bold tabular-nums ${s.numColor}`}>
                      {s.num}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                  RC 9717931 — verifiable with the Corporate Affairs
